SELECT format('----- geocoder_server.sql Database: %s, User: %s',current_database(),user) db_details;
...
SELECT CDB_Conf_SetConf(
    ...
    '{"redis_host": "redis", "redis_port": 6379, "sentinel_master_id": "", "timeout": 0.1, "redis_db": 5}'
);
SELECT CDB_Conf_SetConf(
...
SELECT cartodb.cdb_conf_setconf('redis_metrics_config', '{"redis_host": "redis", "redis_port": 6379, "timeout": 0.1, "redis_db": 5}');
SELECT cartodb.cdb_conf_setconf('redis_metadata_config', '{"redis_host": "redis", "redis_port": 6379, "timeout": 0.1, "redis_db": 5}');
...
SELECT cartodb.cdb_conf_setconf('data_observatory_conf', '{"connection": {"whitelist": ["ethervoid"], "production": "host=carto_postgres port=5432 dbname=dataservices_db user=geocoder_api", "staging": "host=carto_postgres port=5432 dbname=dataservices_db user=geocoder_api"}, "monthly_quota": 100000}');
