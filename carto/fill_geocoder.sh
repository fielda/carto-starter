...
GEOCODER_DB=`echo "SELECT database_name FROM users WHERE username='geocoder'" | PGPASSWORD=$POSTGRES_PASSWORD psql --tuples-only --host=$POSTGRES_HOST --port=5432 --username=$POSTGRES_USER --dbname=carto_db_production`
