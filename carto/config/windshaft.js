var config = {
  ...
  ,port: 8181
  // Listen to all IP addresses (0.0.0.0), not just localhost.
  ,host: '0.0.0.0'
  ...
  ,user_from_host: process.env.USER_FROM_HOST || '^(.*?)\\.'
  ...
  // Templated maps are templated MapConfig documents associated with
  // an authorization certificate. The authorization certificate determines
  // who can instantiate the template and use the resulting map. Authorized
  // users of the instantiated maps will have the same database access
  // privilege of the template owner.
  // "/api/v1/map/named" is the new API,
  // "/tiles/template" is for compatibility with versions up to 1.6.x
  ,base_url_templated: '(?:/api/v1/map/named|/user/:user/api/v1/map/named|/tiles/template)'
  // Base url for the Detached Maps API
  // Detached maps are maps that are configured with a request obtaining a
  // temporary token and then used by referencing the obtained token.
  ...
  // Resource URLs expose endpoints to request/retrieve metadata associated
  // to Maps: dataviews, analysis node status.
  //
  ...
  //  2. {{=it.user}}: will use the username as extracted from `user_from_host` or `routes`.
  //  3. {{=it.port}}: will use the `port` from this very same configuration file.
  ,resources_url_templates: {
    http: process.env.RESOURCE_URL_TEMPLATE_HTTP || 'http://{{=it.user}}.mapsapi.lan/api/v1/map',
    https: process.env.RESOURCE_URL_TEMPLATE_HTTPS || 'https://{{=it.user}}.mapsapi.lan/api/v1/map'
  }
  ...
  ,log_filename: '/Windshaft-cartodb/logs/windshaft.log'
  ...
  ,postgres: {
    ...
    type: 'postgis',
    // user: process.env.POSTGRES_USER || 'publicuser',
    user: 'publicuser',
    // password: process.env.POSTGRES_PASSWORD || 'publicuser',
    password: 'publicuser',
    host: process.env.POSTGRES_HOST || 'carto_postgres',
  ...
  ,renderer: {
    ...
    mvt: {
      //TODO: Find out if using PostGIS is better/faster
    ...
    mapnik: {
      ...
      // Whether grainstore will use a child process or not to transformCartoCSS into Mapnik XML.
  ...
  ,analysis: {
    ...
    batch: {
      ...
      endpoint: process.env.ANALYSIS_ENDPOINT || 'https://carto_sqlapi:8080/api/v2/sql/job',
      // the template to use for adding the host header in the batch api requests
      hostHeaderTemplate: process.env.ANALYSIS_HOST_HEADER_TEMPLATE || '{{=it.username}}.sqlapi.lan'
    },
    logger: {
  ...
  // The ' -S /etc/varnish/secret' option is for securing the connections
  // to the varnish admin console.
  // -- If varnish is started with that option, varnish can be accessed only
  //    by root users (or users with sudoer privileges) through varnishadm.
  // -- If varnish is started without that option, varnish can be accessed
  //    through telnet by anyone who can sign in to the machine.
  // Cartodb communicates with varnish through telnet (some pl/python
  // function). So, if varnish is started with the '-S' option, cartodb
  // cannot connect to varnish. This can cause massive timeouts and delays.
  ,varnish: {
    host: process.env.VARNISH_HOST || 'carto_builder',
    port: process.env.VARNISH_PORT || 6082, // the port for the telnet interface where varnish is listening to
    http_port: process.env.VARNISH_HTTP_PORT || 6081, // the port for the HTTP interface where varnish is listening to
  ...
  ,enabledFeatures: {
    // whether it should intercept tile render errors an act based on them,
    // enabled by default.
    onTileErrorStrategy: true,
    // whether the affected tables for a given SQL must query directly
    // postgresql or use the SQL API
